...
st.set_page_config(page_title="Auditoria FLV - Tome Leve", page_icon="🍎", layout="wide")
...
# ----------------------------------------------------------
# CONFIGURAÇÕES CRÍTICAS E CÉREBRO LÓGICO
# ----------------------------------------------------------
...
def descobrir_familia(nome):
    ...
    # --- ISOLAMENTO ABSOLUTO DE TAXONOMIA ---
        ...
        for _, row in df_loja.iterrows():
            if row['fornecedor'] != current_forn:
                ...
                for cell in ws[ws.max_row]:
                    # Destaca visualmente se for o bloco de itens sem pedido
...
# ----------------------------------------------------------
st.title("🍎 Auditoria FLV Enterprise")
st.write("Motor de Cruzamento de Pedidos e XMLs - V21 com Trava Loja 5 e Detecção de Extras")

col1, col2 = st.columns(2)
with col1:
    st.subheader("1. Planilha de Pedidos")
    arquivo_excel = st.file_uploader("Arraste o Excel de Compras aqui", type=['xlsx'])
with col2:
    st.subheader("2. Notas Fiscais (XML)")
    arquivos_xml = st.file_uploader("Arraste os XMLs dos Fornecedores", type=['xml'], accept_multiple_files=True)
    ...
    if not arquivo_excel or not arquivos_xml:
        ...
        with st.spinner("Sexta-feira processando os dados e rastreando sobras de estoque..."):
            ...
            try:
                ...
                # 3. Cruzamento e Extração de Falhas e Sobras
                registros = []
